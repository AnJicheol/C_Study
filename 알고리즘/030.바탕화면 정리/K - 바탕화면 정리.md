...
## [바로가기](https://school.programmers.co.kr/learn/courses/30/lessons/161990)
...
function solution(wallpaper) {
    var answer = [0,wallpaper[0].length,0,0];
    // x,y 좌표 각각 최대수와 최소수 찾기
    for(var i=0; i<wallpaper.length; i++){ 
        if(wallpaper[wallpaper.length-1-i].indexOf('#') != -1)  answer[0] = wallpaper.length-1-i // x좌표 최소(마지막 인덱스부터 내려감)
        if(wallpaper[i].indexOf('#') < answer[1] && wallpaper[i].indexOf('#') != -1) answer[1] = wallpaper[i].indexOf('#')// indexOf 사용해서 y좌표 최소
        if(wallpaper[i].indexOf('#') != -1)  answer[2] = i+1 // x좌표 최대(첫 인덱스부터 올라감)
        if(wallpaper[i].lastIndexOf('#') > answer[3]) answer[3] = wallpaper[i].lastIndexOf('#') // lastIndexOf 사용해서 y 최대수 찾음
    }
    answer[3] += 1
