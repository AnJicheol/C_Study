...
class Solution {
    public int[] solution(String[] wallpaper) {
        ...
        for(int i = 0; i < wallpaper.length; i++){
            ...
            int l_file = wallpaper[i].indexOf("#");
            int r_file = wallpaper[i].lastIndexOf("#");

            if(i < top && l_file != -1) top = i; 
            if(l_file != -1) bottom = i;
                
            if(l_file < left && l_file != -1) {
                left = l_file;
            }
            
            if(r_file > right && r_file != -1) {
                right = r_file;
            }            
        }
